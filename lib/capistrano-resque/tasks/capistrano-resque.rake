      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            ...
            within current_path do
              execute :rake, %{RAILS_ENV=#{fetch(:rails_env)} QUEUE="#{queue}" PIDFILE=#{pid} BACKGROUND=yes INTERVAL=#{fetch(:interval)} #{"environment" if fetch(:resque_environment_task)} resque:work}
            end
            worker_id += 1
          end
        end
